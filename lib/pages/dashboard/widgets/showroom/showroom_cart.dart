...
class _ChartContentState extends State<ChartContent> {
  ...
  /// 构建柱状图
  Widget buildBarChart(BuildContext context, List<Level1CategoryStat> displayData, Map<int, int> originalIndexMap) {
    final colorScheme = Theme.of(context).colorScheme;

    // 计算最大数量，用于设置纵轴最大值
    final maxCount = displayData.isNotEmpty
        ? displayData.map((e) => e.totalProductsCount).reduce((a, b) => a > b ? a : b)
        : 0;
    final maxY = maxCount > 0 ? (maxCount * 1.2).ceil().toDouble() : 10.0;

    // 计算总数量（只计算有效数据）
    final totalCount = displayData.fold<int>(0, (sum, item) => sum + item.totalProductsCount);
                ...
                BarChartData(
                  ...
                  barTouchData: BarTouchData(
                    ...
                    touchCallback: (barTouchResponse) {
                      ...
                      // 将有效索引转换回原始索引
                      if (touchedIndex >= 0 && touchedIndex < displayData.length) {
                        final originalIndex = originalIndexMap[touchedIndex];
                        if (originalIndex != null && originalIndex < widget.sampleRoomData.length) {
                          setState(() {
                    ...
                    bottomTitles: SideTitles(
                      ...
                      getTitles: (value) {
                        ...
                        if (index >= 0 && index < displayData.length) {
                          ...
                          // 限制长度，避免文字过长
                          final name = item.name.length > 6 ? '${item.name.substring(0, 6)}...' : item.name;
                          return name;
                        }
                        return '';
                      },
                ...
                child: IgnorePointer(
                  child: Row(
                    ...
                    children: displayData.asMap().entries.map((entry) {
                      ...
                      final barHeight = maxY <= 0 ? 0.0 : (count / maxY) * chartAreaHeight;

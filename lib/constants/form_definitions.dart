...
final List<FieldConfig> quoteSampleDefaultFields = [
  ...
  const FieldConfig(label: '中文名称', name: 'product_name_cn', isVisible: true),
  const FieldConfig(label: '英文名称', name: 'product_name_en', isVisible: true),
  const FieldConfig(label: '材质', name: 'material', isVisible: true),
  const FieldConfig(label: '内箱数量', name: 'inner_qty', isVisible: true),
  const FieldConfig(label: '重量', name: 'weight', isVisible: true),
  const FieldConfig(label: '包装方式', name: 'packing', isVisible: true),
  const FieldConfig(label: '外箱数量', name: 'outer_qty', isVisible: true),
  const FieldConfig(label: '体积', name: 'volume', isVisible: true),
  const FieldConfig(label: '尺寸', name: 'size', isVisible: true),
  const FieldConfig(label: '中文描述', name: 'description_cn', isVisible: true),
  const FieldConfig(label: '英文描述', name: 'description_en', isVisible: true),
];
